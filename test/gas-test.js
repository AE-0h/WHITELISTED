...
describe("Gas1", function () {
  ...
  it("add users to whitelist and validate key users are added with correct tier", async function () {
    ...
    let whitelistAddr1 = await gasContract
      .connect(addr1)
      .checkIfWhitelisted(proof1, root1);
    expect(whitelistAddr1).to.equal(true);
    let whitelistAddr2 = await gasContract
      .connect(addr2)
      .checkIfWhitelisted(proof2, root2);
    expect(whitelistAddr2).to.equal(true);
    let whitelistAddr3 = await gasContract
      .connect(addr3)
      .checkIfWhitelisted(proof3, root3);
  ...
  it("whitelist transfer works", async function () {
    ...
    const whiteTransferTx1 = await gasContract
      .connect(addr1)
      .whiteTransfer(recipient1.address, sendValue1, proof1);
    await whiteTransferTx1.wait();
    const whiteTransferTx2 = await gasContract
      .connect(addr2)
      .whiteTransfer(recipient2.address, sendValue2, proof2);
    await whiteTransferTx2.wait();
    const whiteTransferTx3 = await gasContract
      .connect(addr3)
      .whiteTransfer(recipient3.address, sendValue3, proof3);
    ...
    console.log(rec1Balance, rec2Balance, rec3Balance);
    console.log(acc1Balance, acc2Balance, acc3Balance);
  ...
  async function addToWhitelist() {
    ...
    //   await tx3.wait();
    // });
    // addrArray1 = addrArray1.map((x) => keccak256(x));
    // addrArray2 = addrArray2.map((x) => keccak256(x));
    // addrArray3 = addrArray3.map((x) => keccak256(x));
    const merkleTree1 = new MerkleTree(addrArray1, keccak256, {
      hashLeaves: true,
      sortPairs: true,
    });
    root1 = merkleTree1.getHexRoot();
    const leaf1 = keccak256(addr1.address);
    proof1 = merkleTree1.getHexProof(leaf1);
    let tx1 = await gasContract.addToWhitelist(root1, 1);
    await tx1.wait();

    const merkleTree2 = new MerkleTree(addrArray2, keccak256, {
      hashLeaves: true,
      sortPairs: true,
    });
    root2 = merkleTree2.getHexRoot();
    const leaf2 = keccak256(addr2.address);
    proof2 = merkleTree2.getHexProof(leaf2);
    let tx2 = await gasContract.addToWhitelist(root2, 2);
    await tx2.wait();

    const merkleTree3 = new MerkleTree(addrArray3, keccak256, {
      hashLeaves: true,
      sortPairs: true,
    });
    root3 = merkleTree3.getHexRoot();
    const leaf3 = keccak256(addr3.address);
    proof3 = merkleTree3.getHexProof(leaf3);
